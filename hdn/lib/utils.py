...
def extract_patches(x, patch_size, num_patches):
    ...
    for i in tqdm(range(x.shape[0])):
        patches[i * num_patches : (i + 1) * num_patches] = image.extract_patches_2d(
            x[i], (patch_size, patch_size), max_patches=num_patches, random_state=i
        )
    return patches
...
def _pad_crop_img(x, size, mode) -> torch.Tensor:
    ...
    if cond:
        raise ValueError(
            "trying to {} from size {} to size {}".format(mode, x_size, size)
        )
...
def plotProbabilityDistribution(
    signalBinIndex, histogram, gaussianMixtureNoiseModel, min_signal, max_signal, n_bin
):
    ...
    queryObservations = torch.from_numpy(queryObservations_numpy).float()
...
def get_normalized_tensor(img, model):
    """
    ...
    """
    test_images = torch.from_numpy(img)
