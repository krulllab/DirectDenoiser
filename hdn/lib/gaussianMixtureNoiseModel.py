...
class GaussianMixtureNoiseModel(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if(kwargs.get('params') is None):
            ...
            if (weight is None):
                ...
                weight = torch.from_numpy(weight.astype(np.float32)).float()
    ...
    def polynomialRegressor(self, weightParams, signals):
        """Combines `weightParams` and signal `signals` to regress for the gaussian parameter values.
                ...
                Parameters
                ----------
                weightParams : torch.FloatTensor
    ...
    def normalDens(self, x, m_= 0.0, std_= None):
        """Evaluates the normal probability density at `x` given the mean `m` and standard deviation `std`.
                ...
                x: torch.FloatTensor
                    Observations
                m_: torch.FloatTensor
                    Mean
                std_: torch.FloatTensor
                    Standard-deviation
                Returns
                -------
    ...
    def getGaussianParameters(self, signals):
        """Returns the noise model for given signals
                ...
                Returns
                -------
                noiseModel: list of torch.FloatTensor
    ...
    def train_(self, signal, observation, learning_rate=1e-1, batchSize=250000, n_epochs=2000, name= 'GMMNoiseModel.npz', lowerClip=0, upperClip=100):
        ...
        for t in range(n_epochs):
            ...
            observations = torch.from_numpy(observations.astype(np.float32)).float()
