...
class LadderVAE(nn.Module):
    def __init__(
        ...
        if self.likelihood_form == "gaussian":
            ...
            self.likelihood = NoiseModelLikelihood(
                n_filters, color_ch, data_mean, data_std, noiseModel
            )
        else:
    ...
    @torch.no_grad()
    def predict(self, x, n_samples, batch_size):
        """
        Performs bottom up pass once but topdown pass n_samples times.
        :param x: input tensor
        :param n_samples: number of samples to generate
        :param batch_size: number of samples to generate at a time
        ...
        for _ in range(n_batches):
            batch = [
                bu_value.repeat_interleave(batch_size, 0) for bu_value in bu_values
            ]
            out, _ = self.topdown_pass(batch)
            out = crop_img_tensor(out, img_size)
            _, likelihood_info = self.likelihood(out, None)
            s_hat = likelihood_info["mean"]
            samples.append(s_hat)

        remainder = n_samples % batch_size
        if remainder > 0:
            final_batch = [
                bu_value.repeat_interleave(remainder, 0) for bu_value in bu_values
            ]
            out, _ = self.topdown_pass(final_batch)
            out = crop_img_tensor(out, img_size)
            _, likelihood_info = self.likelihood(out, None)
            s_hat = likelihood_info["mean"]
            samples.append(s_hat)
